document.addEventListener('DOMContentLoaded', () => {
    ...
    const initGlobalConfigs = () => {
        ...
        if (socialContainer) {
            socialContainer.innerHTML = Object.entries(GLOBAL_CONFIG.social).map(([key, url]) => 
                `<a href="${url}" target="_blank" rel="noopener noreferrer" aria-label="${key}">${getSocialIcon(key)}</a>`
            ).join('');
        }
        ...
        if (devEl) {
            devEl.innerHTML = `<a href="${GLOBAL_CONFIG.developer.url}" target="_blank" rel="noopener noreferrer">${GLOBAL_CONFIG.developer.text}</a>`;
        }
        
        // Contact Info (if elements exist)
        ...
        if(catalogLink) catalogLink.href = GLOBAL_CONFIG.catalog.url;
    };
    ...
    const initMobileMenu = () => {
        ...
        const catalogButton = document.querySelector('#catalog-link').cloneNode(true);
